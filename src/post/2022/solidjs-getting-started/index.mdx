...
まずは、基本機能を試してみるために小さなアプリを作ってみるだけなのですべての機能までは試さない。
...
<LinkCard url="https://2021.stateofjs.com/ja-JP/libraries/front-end-frameworks/" />
...
Todoリストの表示部分には、`<For>`コンポーネントという制御フローコンポーネントがあるのでそれを使っている。
`map`だと全体が再レンダリングされて非効率なので、こういう専用コンポーネントが用意してるとのこと。
...
動的にクラスを当てたい場合、`classList`というSXのカスタム属性があるのでそれを使う。
...
注意点として、追跡スコープの外でpropsをdestructureするとリアクティブではなくなる。
...
Todoリストのストアを作ってみる。
`createStore`で作れる。readonlyなプロキシオブジェクトを返す。
